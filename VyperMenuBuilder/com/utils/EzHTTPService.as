package utils {
	...
	public class EzHTTPService {
		...
		public function myCallback(event:*):void { // NOTE - event data type here is "*" to allow soft errors to be redirected into the incoming data stream...
			...
			try {
				for (var i:int = 0; i < ac.length; i++) {
					ac.source[i] = JSON.decode(ac.source[i],function(e:FaultEvent):void {});
				}
			...
			if (this.queue.length > 0) {
				...
				return this.invokeHTTPService(o.url, o.request.data, o.callback, o.resultFormat);
			} else {
				this.isBusy = false;
			}
		}
		...
		public function onFaultHandler(event:FaultEvent):void {
			var sEX:String = (new ObjectExplainer(this.requestObj)).explainThisWay("\n");
			...
			//trace('EzHTTPService.onFaultHandler().1 --> msg='+msg);
			if (this.alertShowFunc is Function) {
				this.alertShowFunc(msg, title);
			} else {
				mx.controls.Alert.show(StringUtils.ellipsis(msg,1200), title);
			}
		}
		
		private function myFaultCallback(event:FaultEvent):void {
			//trace('EzHTTPService.myFaultCallback().1 --> event='+event.toString());
			if (this.faultCallBack is Function) {
				//trace('EzHTTPService.myFaultCallback().2 !');
				this.faultCallBack(event);
			} else {
				//trace('EzHTTPService.myFaultCallback().3 !');
		...
		private function invokeHTTPService(url:String, data:String, callback:Function, resultFormat:String = this.arrayResultType):void {
			var title:String = "";
			var msg:String = "";
		...
		public function post(url:String, data:String, callback:Function, resultFormat:String = this.arrayResultType):void {
		...
		public function deepCopyQueue():Array {
			return ArrayUtils.deepCopyFrom(this.queue);
		}
