package vzw.controls.colorPicker
{
    ...
    public class ColorSwatch extends Canvas
    {
        ...
        private function onDown(e:MouseEvent):void{
            ...
            tmpDta.draw(picker);
            var isHit:Boolean = spectrum.hitTestPoint(e.stageX,e.stageY,true);
            //trace(this+'.onDown().1 --> (' + isHit1 + ') parent.mouseX,parent.mouseY='+parent.mouseX+','+parent.mouseY);
            //trace(this+'.onDown().2 --> (' + isHit2 + ') e.localX,e.localY='+e.localX+','+e.localY);
            //trace(this+'.onDown().3 --> (' + isHit3 + ') e.stageX,e.stageY='+e.stageX+','+e.stageY);
            //trace(this+'.onDown().4 --> spectrum.x,spectrum.y='+spectrum.x+','+spectrum.y);
            //trace(this+'.onDown().5 --> spectrum.width,spectrum.height='+spectrum.width+','+spectrum.height);
            //trace(this+'.onDown().6 --> spectrum.contentMouseX,spectrum.contentMouseY='+spectrum.contentMouseX+','+spectrum.contentMouseY);
            if (isHit) {
