package baseflex.utils.displayObject {
	...
	import baseflex.utils.strings.Strings;
	...
	public class imgLoader extends UIComponent {
		...
		private function rslFix(url:String, start:String ,stop:String ,key:String):String {
			return url.substring(0,url.split(key)[0].toString().lastIndexOf(start)+1)+baseflex.utils.strings.Strings.filter(url.split(key)[1].substr(url.split(key)[1].toString().indexOf(stop)),stop,"");
		}
