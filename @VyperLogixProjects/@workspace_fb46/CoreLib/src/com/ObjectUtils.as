package com {
	...
	public class ObjectUtils {
		...
		public static function add(obj1:*,obj2:*):void {
			var keys:Array = ObjectUtils.keys(obj1);
			for (var i:int = 0; i < keys.length; i++) {
				obj1[keys[i]] += obj2[keys[i]];
			}
		}
		
		public static function is_equal(obj1:*,obj2:*):Boolean {
			var keys1:Array = ObjectUtils.keys(obj1);
			var keys2:Array = ObjectUtils.keys(obj2, 
				function (data:*,key:*):Boolean {
					if ( (keys1.indexOf(key) > -1) && (obj1[key] == data) ) {
						return true;
					}
					return false;
				}
			);
			return ((keys1.length + keys2.length) > 0) && (keys1.length == keys2.length);
		}
		
		public static function is_equal_for_criteria(obj1:*,obj2:*,criteria:Function):Boolean {
			var keys1:Array = ObjectUtils.keys(obj1,criteria);
			var keys2:Array = ObjectUtils.keys(obj2, 
				function (data:*,key:*):Boolean {
					if ( (keys1.indexOf(key) > -1) && (obj1[key] == data) ) {
						return true;
		...
		public static function has_key(anObject:Object,key:String):Boolean {
			var keys:Array = ObjectUtils.keys(anObject);
			return ( (keys.length > 0) && (keys.indexOf(key) > -1) );
		}
		...
		public static function intoArrayCollection(source:Object):ArrayCollection {
			var ar:Array = [];
			var keys:Array = keys(source);
			var item:*;
			for (var i:int = 0; i < keys.length; i++) {
				item = {};
		...
		public static function keys(data:Object,criteria:Function=null):Array {
			...
			try {
				for (var i:String in data) {
					if ( (criteria == null) || ( (criteria is Function) && (criteria(data[i],i)) ) ) {
						a.push(i);
					}
				}
			} catch (e:Error) {}
		...
		public static function keys_filtered(data:Object,filter:Function=null):Array {
			...
			try {
				var key:String;
				for (var i:String in data) {
					key = (filter is Function) ? filter(i) : i;
					if (key) {
						a.push(key);
					}
				}
		...
		public static function keys_filtered_by(data:Object,filter:Function=null):Array {
			var a:Array = [];
			try {
				for (var i:String in data) {
					if (filter(i)) {
		...
		public static function indexOf(source:*,target:*,using_any_part_of:Boolean=false):int {
			var anObj:*;
			var aKey:String;
			var keys1:Array;
			var keys2:Array;
			...
			try {
				for (var i:int = 0; i < source.length; i++) {
					...
					if ( (keys1.length > 0) && (keys2.length > 0) ) {
						isEqual = (using_any_part_of) ? false : true;
						for (j in keys1) {
							aKey = keys1[j];
							if ( (!using_any_part_of) && (keys2.indexOf(aKey) == -1) ) {
								isEqual = (using_any_part_of) ? isEqual : false;
								break;
							} else {
								aValue1 = anObj[aKey];
								aValue2 = target[aKey];
								if ( (!using_any_part_of) && (aValue1 != aValue2) ) {
									isEqual = false;
									break;
								} else if ( (using_any_part_of) && (aValue1 == aValue2) ) {
									isEqual = true;
									break;
								}
							}
