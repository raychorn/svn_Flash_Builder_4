package com {
	...
	public class CustomErrorTips {
		...
		public static function set_tooltip_color_for(styleName:String, color:String, borderColor:String):void {
			var cssObj:CSSStyleDeclaration;
			cssObj = FlexGlobals.topLevelApplication.styleManager.getStyleDeclaration(styleName)
			if ( (borderColor is String) && (borderColor.length > 0) ) {
				cssObj.setStyle("borderColor", borderColor);
			}
		...
		public static function set_tips_color(value:String,color:String=null):void {
			...
			var aColor:* = CustomErrorTips._colors[value];
			if (aColor) {
				if (aColor['color']) {
					aColor = aColor['color'];
				} else {
				...
				set_tooltip_color_for('.errorTip',aColor,value);
			}
		}
			...
			for (var parentDocument:* in rect_parents_dict) {
				...
				if (aBucket.length > 2) {
					...
					if (parentDocument.parentDocument) {
						container = parentDocument.parentDocument.getChildByName(container_name) as Group;
...
//			}
		...
		public static function createToolTip(text:String,target:*,position:Point,tip_orientation:String,use_contentToLocal:Boolean=true):ToolTip {
			...
			if (CustomErrorTips._valid_tip_orientations_.indexOf(tip_orientation) > -1) {
				...
				catch (err:Error) {trace(DebuggerUtils.getFunctionName(new Error())+'.ERROR.1 '+err.toString()+', '+err.getStackTrace());}
				trace(StringUtils.repeatedChars('=',30)+'\n');
			} else {
