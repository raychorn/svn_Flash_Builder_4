package com {
	...
	public class StringUtils {
		...
		public static function zeroFill(myNumber:Number):String {
		...
		public static function decimalToPercentage(decimalValue:Number,precision:int=2):String {
			var percentage:Number = decimalValue * 100.0;
			var roundedPercentage:Number = Math.round(percentage);
			var numberAsString:String = (roundedPercentage < 100.0) ? roundedPercentage.toPrecision(precision) : "100";
			var percentAsString:String = numberAsString + "%";
			
			return percentAsString;
		}
		
		public static function milliseconds_as_time(ms:Number):String {
			var ms_per_sec:Number = 1000;
			var num_secs_or_mins:Number = 60;
			var ms_per_hour:Number = ms_per_sec * num_secs_or_mins * num_secs_or_mins;
			var ms_per_min:Number = ms_per_hour / num_secs_or_mins;
			return Math.floor(ms / ms_per_hour) + ":" + zeroFill(Math.floor(ms / ms_per_min) % num_secs_or_mins) + ":" + zeroFill(Math.floor(ms / ms_per_sec) % num_secs_or_mins);
		}
		...
		public static function has_repeatedChars(source:String, aChar:String):Boolean {
			var num:int = 0;
			var ch:String;
		...
		public static function urlEncode(plaintext:String):String {
			...
			var charCode:uint;
			var encoded:String = "";
			if ( (plaintext is String) && (plaintext.length > 0) ) {
