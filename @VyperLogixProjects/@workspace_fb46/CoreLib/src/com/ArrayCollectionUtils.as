package com {
	...
	public class ArrayCollectionUtils {
		...
		public static function iterate(target:ArrayCollection, aFunc:Function, aSelector:String):void {
			if (aFunc is Function) {
				var i:int;
				var anItem:Object;
				for (i = 0; i < target.length; i++) {
		...
		public static function iterate_until(target:ArrayCollection, aFunc:Function, aSelector:String):void {
			if (aFunc is Function) {
				var i:int;
				var anItem:Object;
				var bool:Boolean;
		...
		public static function findIndexOfItemCaseless(dp:*, selector:*, pattern_or_func:*):int {
			var i:int;
			var ac:ArrayCollection;
			if (dp is ArrayCollection) {
				ac = dp;
			...
			var is_func:Boolean = (pattern_or_func is Function);
			if ( (ac != null) && (pattern_or_func != null) ) {
				var obj:*;
				if (!is_func) {
					pattern_or_func = pattern_or_func.toLowerCase();
				}
				var matches:Boolean = false;
				for (i = 0; i < ac.length; i++) {
					obj = ac.getItemAt(i);
					if (is_func) {
						matches = pattern_or_func((selector is String) ? obj[selector] : selector,obj);
						if (matches) {
							return i;
						}
					} else {
						if ( ((obj is String) == false) && (selector != null) && (selector is String) ) {
							if (String(obj[selector]).toLowerCase() == pattern_or_func) {
								return i;
							}
						} else {
							if (String(obj).toLowerCase() == pattern_or_func) {
								return i;
							}
						}
					}
