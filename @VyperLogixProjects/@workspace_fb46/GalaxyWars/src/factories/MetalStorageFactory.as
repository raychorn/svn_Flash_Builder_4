package factories {
	...
	public class MetalStorageFactory extends ResourceFactory {

		/*
		Constructor is not used... 
		*/
		public function MetalStorageFactory():* {
		}
		
		[Bindable]
		public static var name:String = 'MetalStorage';
		...
		public static function get current_level():int {
			if (__current_level__ < 0) {
				...
				__current_level__ = app.mySO.data.__metal_storage_level__;
			}
		...
		public static function next_level():void {
			__current_level__++;
			var app:GalaxyWars = FlexGlobals.topLevelApplication as GalaxyWars;
			app.mySO.data.__metal_storage_level__ = __current_level__;
			app.mySO.flush();
		}
		...
		public static function level_production_rate_for(level:int):Object {
			...
			var capacity:Number = resources[MetalFactory.name] * 24*7;
		...
		public static function level_upgrade_cost_for(level:int):Object {
			try {
				...
				var metal_value:Number = ResourceFactory.resource_value_ceiling(resources[metal]*50.0);
				return {
					metal: metal_value
				};
			} catch (err:Error) {}
