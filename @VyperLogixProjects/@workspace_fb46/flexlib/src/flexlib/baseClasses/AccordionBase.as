...
package flexlib.baseClasses
{
  ...
  public class AccordionBase extends Container implements IHistoryManagerClient, IFocusManagerComponent
  {
    ...
    override public function styleChanged(styleProp:String):void
    {
        ...
        if (headerStyleName)
        {
          if (FlexVersion.compatibilityVersion < FlexVersion.VERSION_3_0)
          {
            var headerStyleDecl:CSSStyleDeclaration =
				IStyleManager2(FlexGlobals.topLevelApplication.styleManager).getStyleDeclaration("." + headerStyleName);
            if (headerStyleDecl)
            {
      ...
      else if (IStyleManager2(FlexGlobals.topLevelApplication.styleManager).isSizeInvalidatingStyle(styleProp))
      {
    ...
    private function createHeader(content:DisplayObject, i:int):void
    {
      ...
      if (headerStyleName)
      {
        if (FlexVersion.compatibilityVersion < FlexVersion.VERSION_3_0)
        {
          var headerStyleDecl:CSSStyleDeclaration = IStyleManager2(FlexGlobals.topLevelApplication.styleManager).getStyleDeclaration("." + headerStyleName);
