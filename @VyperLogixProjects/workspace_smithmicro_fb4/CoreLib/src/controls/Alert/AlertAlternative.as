package controls.Alert {
	...
	public class AlertAlternative {
		...
		http://fatal-exception.co.uk/blog/?p=69#more-69
		...
		textSize:int			-	size of the text, expressed as <font face="Verdana" size="+1"></font> 
		textFont:String			-	Verdana or the like 
		...
		*/
		...
		private static function _show(Text:* = "Made by Vyper Logix Corp.", ALERTOPTIONS:Object = null):void {
			if (stage == null) {
		...
		public static function show(Text:* = "Made by Vyper Logix Corp.", ALERTOPTIONS:Object = null):void {
			AlertAlternative._show(Text,ALERTOPTIONS);
		}

		public static function error(Text:* = "Made by Vyper Logix Corp.", ALERTOPTIONS:Object=null):void {
			var colour:uint = 0xff0000;
			var has_colour:Boolean = false;
			ALERTOPTIONS = (ALERTOPTIONS) ? ALERTOPTIONS : {'colour':colour};
			try {
				colour = ALERTOPTIONS['colour'];
				has_colour = true;
			} catch (err:Error) {}
			if (!has_colour) {
				ALERTOPTIONS['colour'] = colour;
			}
			trace(Text);
			AlertAlternative._show(Text,ALERTOPTIONS);
		}
		...
		private static function createButtonTextField(Text:String, alertOptions:AlertOptions):TextField {
			...
			Text = "<b>"+Text+"</b>";
			myTextField.htmlText = '<font face="'+alertOptions.textFont+'" size="'+alertOptions.textSize_asHTML+'">'+Text+'</font>';
...
import controls.Alert.mxSurpriseImage;
...
internal class AlertOptions {
	...
	public function get textSize_asHTML():String {
		return ((this.textSize > 0) ? '+' : ((this.textSize < 0) ? '-' : ''))+this.textSize;
	}
	...
	public function AlertOptions(alertOptions:Object, Text:*):void {
		...
		text = String(Text); // StringUtils.fit_to_width(Text.toString(),40);
	}
}