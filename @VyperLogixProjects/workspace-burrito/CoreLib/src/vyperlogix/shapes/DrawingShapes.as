package vyperlogix.shapes {
	...
	public class DrawingShapes {
		...
		public function DrawingShapes() {
			throw new ArgumentError( "The DrawingShapes Class cannot be instantiated.");
		}
		
		public static function get_handle():Object {
		 ...
		 * dash.  Default = 5
		 */
		public static function drawDash(handle:DrawingHandle,target:Graphics, x1:Number,y1:Number,x2:Number,y2:Number,dashLength:Number = 5, spaceLength:Number = 5 ) : void {
			...
			handle.issue_callback(new Point(dashX, dashY));
			handle.issue_callback(new Point(spaceX, spaceY));
			
			handle.init_verticies(1);
			target.moveTo( x1, y1 );
			handle.issue_callback(new Point(x1, y1));
			while (hyp > 0) {
				x1 += dashX;
				y1 += dashY;
				hyp -= dashLength;
			...
			target.moveTo( x2, y2 );
			handle.issue_callback(new Point(x2, y2));
			handle.issue_callback(null);
		}
		 ...
		 */
		public static function drawArc(handle:DrawingHandle,target:Graphics,x:Number,y:Number,radius:Number,arc:Number,startAngle:Number = 0,yRadius:Number = 0) : void {
			...
			if (segs > 0) {
				target.moveTo( x, y );
				handle.issue_callback(new Point(x, y));
		 ...
		 */
		public static function drawWedge(handle:DrawingHandle,target:Graphics,x:Number,y:Number,radius:Number,arc:Number,startAngle:Number = 0,yRadius:Number = 0) : void {
			...
			if (segs > 0) {
				...
				target.lineTo( ax, ay );
				handle.issue_callback(new Point(ax, ay));
		 ...
		 */
		public static function drawStar(handle:DrawingHandle,target:Graphics,x:Number,y:Number,points:uint,innerRadius:Number,outerRadius:Number,angle:Number = 0) : void {
			...
			if (points > 2) {
				handle.init_verticies(int(points));
				// init vars
				...
				var pt:Point = new Point(x + (Math.cos( start ) * outerRadius),y - (Math.sin( start ) * outerRadius));
				target.moveTo( pt.x, pt.y);
				handle.issue_callback(pt);
				// draw lines
				for (n = 1; n <= points; ++n) {
					dx = x + Math.cos( start + (step * n) - halfStep ) * innerRadius;
					dy = y - Math.sin( start + (step * n) - halfStep ) * innerRadius;
					target.lineTo( dx, dy );
					handle.issue_callback(new Point(dx, dy));
		 ...
		 */
		public static function drawPolygon(handle:DrawingHandle,target:Graphics,x:Number,y:Number,sides:uint,radius:Number,angle:Number = 0) : void {
			...
			if (sides > 2) {
				handle.init_verticies(int(sides));
				// init vars
				...
				// calculate starting angle in radians
				start = (angle / 180) * Math.PI;
				var pt:Point = new Point(x + (Math.cos( start ) * radius), y - (Math.sin( start ) * radius));
				target.moveTo(pt.x,pt.y);
				handle.issue_callback(pt);
				// draw the polygon
				for (n = 1; n <= sides; ++n) {
		 ...
		 */
		public static function drawBurst(handle:DrawingHandle,target:Graphics,x:Number,y:Number,sides:uint,innerRadius:Number,outerRadius:Number,angle:Number = 0 ) : void {
		 ...
		 */
		public static function drawGear(handle:DrawingHandle,target:Graphics,x:Number,y:Number,sides:uint,innerRadius:Number = 80,outerRadius:Number = 4,angle:Number = 0,holeSides:Number = 2,holeRadius:Number = 0) : void {
			...
			if (sides > 2) {
				...
				if (holeSides > 2) {
					...
					pt = new Point(x + (Math.cos( start ) * holeRadius), y - (Math.sin( start ) * holeRadius));
					target.moveTo(pt.x,pt.y);
					handle.issue_callback(pt);
		 ...
		 * 
		 */
		public static function drawLine(handle:DrawingHandle,target:Graphics,x:Number,y:Number,length:Number,direction:String = DrawingShapes.HORIZONTAL_LINE) : void {
			handle.init_verticies(1);
			target.moveTo( x, y );
			handle.issue_callback(new Point(x, y));
			switch (direction) {
				case DrawingShapes.HORIZONTAL_LINE :
					...
					handle.issue_callback(new Point(length, y));
					break;
				case DrawingShapes.VERTICAL_LINE :
					target.moveTo( x, y );
					handle.issue_callback(new Point(x, y));
					target.lineTo( x, length );
					handle.issue_callback(new Point(x, length));
					break;
			}
			handle.issue_callback(null);
		}
